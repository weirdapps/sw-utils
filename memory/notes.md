...
- **Remaining:** A2 scouting (needs per-GAC-round opponent ally) · A4 events (swgohevents ICS) · **kill META scrape** (swgoh.gg API key → compute_teams fully browser-free) · **Track B PvE farming macro** (Android emulator on Mac — the grind-killer). Branch `swgoh-automation` (6 commits) **NOT merged to master**. Cleanup: `~/Downloads/comlink-macos*` (broken Mac binary) is junk; keep `comlink-openapi.json` as reference.
- **Cosmetic TODO:** gap-unit names show as baseIds for UNOWNED units (name_type_map only has owned 397) → refresh full baseId→name from comlink `getGameData`+`getLocalization`.
