...
def test_tiers_follow_the_stated_order():
    ...
    # Owner, 2026-09-06: "optimize my mods first to arena then grand arena then rote
    # then tw order of priority." The whole arena block (deployed wall 1, climb 2,
    # fleet 3) now sits ABOVE the GAC block, which is a defense block (4-5) then an
    # offense block (6-7). This re-reverses the 2026-08-24 call that had put the whole
    # of Squad Arena below GAC. See the ladder comment in invest_plan.py: the rung has
    # flipped three times because Squad Arena and GAC want the same five units.
    tiers = {e["unit"]: e["tier"] for e in ip.priority_units(roster, board, arena, rote)}
    assert tiers == {"ARENADEF": 1, "ARENAOFF": 2, "WALL": 4, "ATK": 6,
...
def test_best_tier_wins_when_a_unit_fills_several_roles():
    ...
    # in Squad Arena. Best-tier-wins hands it SQUAD ARENA DEFENSE, because the arena
    # block went back above the GAC block on 2026-09-06. All three roles stay visible,
    # and the rate must be the winning rung's, not the 50% 5v5 hold or the 96% 3v3 clear.
    ...
    assert out[0]["tier"] == ip.ARENA_DEFENSE_TIER  # arena wall, not GAC 5v5 defense
    assert out[0]["tier"] < min(t for t, _r, _l in ip.BOARD_ROLES if _l.startswith("GAC"))
    assert out[0]["rate"] == 31                     # arena's hold, not the 5v5 wall's 50
...
def test_squad_arena_outranks_gac():
    # Owner, 2026-09-06: "optimize my mods first to arena then grand arena then rote
    # then tw order of priority." Both modes want the same five units, so whichever
    # rung is first silently owns the contested speed arrows. This pins the direction
    # so a future edit cannot quietly flip it back; it is the third setting of the
    # rung, and only the owner decides it.
    assert ip.ARENA_FLEET_TIER < min(t for t, _r, _l in ip.BOARD_ROLES
    ...
    assert order.index("ARENADEF") < order.index("WALL")
...
def test_relic_queue_uses_the_roster_scale_and_keeps_priority_order():
    board = _board(**{"5v5": {"defense": [_sq(50, "WALL")], "offense": [_sq(96, "ATK")]}})
    arena = {"defense": _sq(0, "ARENADEF")}
    roster = _roster(("WALL", {"rt": 8}),        # displayed R6 -> below target, tier 4
                     ("ATK", {"rt": 9}),         # displayed R7 -> AT target, excluded
                     ("ARENADEF", {"rt": 7}))    # displayed R5 -> below, arena tier 1
    priority = ip.priority_units(roster, board, arena)
    q = ip.relic_queue(roster, priority, ip.rt_for_displayed_relic(7))
    assert [e["unit"] for e in q] == ["ARENADEF", "WALL"]   # arena first, GAC wall after
...
def test_mod_priority_order_is_a_duplicate_free_character_only_ordering():
    ...
    assert order == ["GL", "WALL", "ATK"]      # tier 4 by rate, then tier 6
...
def test_ability_queue_drops_unowned_units_and_sorts_by_priority():
    ...
    assert out[1]["mode"] == "grand-arena"           # arena now sorts ABOVE GAC
